...
> 版本: 1.0 | 日期: 2026-06-25 | 基于: PRD v1.8 + 两次盘问决策 | 状态: Draft

---

## Problem Statement

v0.2.x 解决了检测架构的根本问题——A11y 优先 + 轮询兜底 + 补偿机制在 MIUI 上稳定运行，额度不再异常波动。但用户体验层面仍有三个痛点未解决：

1. **蒙层拦截触控** — 蒙层覆盖时用户无法正常操作短视频 App（点赞/滑动/评论），与本项目的"非对抗性劝导"设计哲学矛盾。
2. **被干预时没有节奏感** — 额度耗尽后用户只能等它自然恢复到 5 以上才能解除，过程中缺乏清晰的视觉反馈和控制手段。
3. **费率不匹配个人作息** — 默认的白天/夜间划分和消耗速率是固定的，作息不规律的用户无法按自己的节奏调节。

此外，通知栏、悬浮球、设置等交互细节仍停留在最小可用状态，缺少 Android 用户习惯的基础交互（如通知可点击、设置可自定义）。

## Solution

v0.3.x 通过 8 个功能（2 个 P0、6 个 P1）解决上述问题，核心变化是：**从"能用"到"好用"**。

**蒙层重构**（P0）是这一阶段的核心——从单层 WindowManager View 拆分为"视觉层 + 按钮层"两层，实现真实触控穿透。用户被干预时仍可正常操作短视频 App，只是视觉上"看不下去"。

**通知栏操作**（P0）补齐了 Android 服务类 App 的基础交互——用户不打开 App 也能查看额度和暂停服务。

**四项体验优化**（P1）——冷却机制、事件驱动隐藏、蒙层效果配置、时段费率调节——将控制权交还给用户，让"非对抗性"不再只是设计哲学，而是可感知的体验。
...
### 额度冷却
5. 作为一个额度耗尽后退出的用户，我希望切回短视频时额度恢复到 5 以上才解除蒙层，以便不会在低额度反复进出。
6. 作为一个被蒙层打断后想继续刷的用户，我希望关闭蒙层后有 30 秒防打扰期，以便不会被频繁打断。
7. 作为一个防打扰期结束还在刷的用户，我希望蒙层重新出现，以便保持有节奏的提醒。
...
### 悬浮球
12. 作为一个宽限期的用户，我希望悬浮球在宽限期间显示倒计时和外圈进度环递减，以便直观感知剩余缓冲时间。
13. 作为一个不想被悬浮球一直打扰的用户，我可以在设置中选择"仅在看短视频时显示悬浮球"，以便桌面或其他 App 时不被干扰。
...
### 事件响应
17. 作为一个退出短视频 App 的用户，我希望蒙层立即消失，以便不会在桌面上还盖着一层漂白。
18. 作为一个切换到其他 App 的用户，我希望蒙层不会在非短视频界面上出现，以便不打扰正常使用。
