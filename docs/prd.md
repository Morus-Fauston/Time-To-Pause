...
- **蒙层**：非对抗性视觉干预。可选两种方向——**温和干预**（渐变呼吸动画，强调自我觉察）或**视觉剥夺**（漂白+背景模糊+动态噪点，各三档强度）。不拦截下层触控。触发后带冷却机制——额度必须恢复到 5 以上才能解除，遏制低额度反复进出。
...
30. 作为一个作息不规律的用户，我希望自定义白天/黑夜的时段划分和消耗/恢复速率，以便额度调节匹配我的实际生活节奏。
...
- **阻遏冷却机制**——蒙层触发阈值 0，解除阈值 5。触发后额度必须恢复到 5 以上才能解除。冷却期内切出再切入仍继续阻遏。
...
| 阻遏冷却机制 | 触发阈值 0 / 解除阈值 5，冷却期内切入立即阻遏 | P1 |
| 事件驱动蒙层隐藏 | 退出短视频时 A11y 事件即时隐藏蒙层，不等 tick 窗口 | P1 |
| 蒙层方向与效果可配置 | 温和干预(渐变呼吸) / 视觉剥夺(漂白+模糊+噪点，各三档) | P1 |
| 时段与费率可调节 | 30 分钟粒度时间轴滑块调节白天范围，四个费率输入框 | P1 |
